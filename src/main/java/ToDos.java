...
public class ToDos {
     ...
     */
    public Task[] search(String query) { // массив стоит на первом месте, потом метод и в скобках параметр
        ...
        for (Task task : tasks) { // перебираем все задачи
            if (task.matches(query)) { // если задача подходит под запрос
                result = addToArray(result, task); // добавляем её в массив ответа. В массив кладем метод.
            }
        }
        return result;
    }
